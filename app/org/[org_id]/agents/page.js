...
function Home({ params, searchParams, isEmbedUser }) {
  ...
  const handleRowHover = (row) => {
    ...
    if (!prefetchedRoutes.current.has(routeKey)) {
      ...
      const prefetchUrl = `/org/${resolvedParams.org_id}/agents/configure/${row._id}?version=${row.versionId}&type=${agentType}`;
  ...
  const onClickConfigure = (id, versionId, agentType = "api") => {
    ...
    router.push(`/org/${resolvedParams.org_id}/agents/configure/${id}?version=${versionId}&type=${agentType}`);
  };
